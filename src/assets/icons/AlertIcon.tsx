import type { IconProps } from '../../types';

interface AlertIconProps extends IconProps {
  isActive?: boolean;
}

const AlertIcon: React.FC<AlertIconProps> = ({
  size = 24,
  isActive = false,
  className,
  ...props
}) => {
  const colors = isActive
    ? { main: '#003240' }
    : { main: '#A3EBFF' };

  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      {...props}
    >
      <path
        d="M12.02 2.91016C8.71 2.91016 6.02 5.60016 6.02 8.91016V11.8002C6.02 12.4102 5.76 13.3402 5.45 13.8602L4.3 15.7702C3.59 16.9502 4.08 18.2602 5.38 18.7002C9.69 20.1402 14.34 20.1402 18.65 18.7002C19.86 18.3002 20.39 16.8702 19.73 15.7702L18.58 13.8602C18.28 13.3402 18.02 12.4102 18.02 11.8002V8.91016C18.02 5.61016 15.32 2.91016 12.02 2.91016Z"
        stroke={colors.main}
        strokeWidth="1.5"
        strokeMiterlimit="10"
        strokeLinecap="round"
      />
      <path
        d="M13.87 3.20141C13.56 3.11141 13.24 3.04141 12.91 3.00141C11.95 2.88141 11.03 2.95141 10.17 3.20141C10.46 2.46141 11.18 1.94141 12.02 1.94141C12.86 1.94141 13.58 2.46141 13.87 3.20141Z"
        stroke={colors.main}
        strokeWidth="1.5"
        strokeMiterlimit="10"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        opacity="0.4"
        d="M15.02 19.0586C15.02 20.7086 13.67 22.0586 12.02 22.0586C11.2 22.0586 10.44 21.7186 9.9 21.1786C9.36 20.6386 9.02 19.8786 9.02 19.0586"
        stroke={colors.main}
        strokeWidth="1.5"
        strokeMiterlimit="10"
      />
    </svg>
  );
};

export default AlertIcon;
