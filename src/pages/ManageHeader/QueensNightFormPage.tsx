  ...
  const handleSave = (subItemData: Partial<HeaderSubItem>) => {
    ...
    categories = categories.map(category => {
      if (category.id === categoryId) {
        ...
        if (subItemId && subItemId !== 'new') {
          newSubItems = newSubItems.map(item => item.id === subItemId ? { ...item, ...subItemData, pageType: 'queens-night' } as HeaderSubItem : item);
        } else {
          newSubItems.push({ ...subItemData, id: Date.now().toString(), isHidden: false, pageType: 'queens-night' } as HeaderSubItem);
        }
