  ...
  const handleSave = (subItemData: Partial<HeaderSubItem>) => {
    ...
    categories = categories.map(category => {
      if (category.id === categoryId) {
        ...
        if (subItemId && subItemId !== 'new') {
          newSubItems = newSubItems.map(item => item.id === subItemId ? { ...item, ...subItemData, pageType: 'team-parties' } as HeaderSubItem : item);
        } else {
          newSubItems.push({ ...subItemData, id: Date.now().toString(), isHidden: false, pageType: 'team-parties' } as HeaderSubItem);
        }
    ...
    <div className="p-6 text-white min-h-screen">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">
          {subItemId && subItemId !== 'new' ? 'Edit Team Parties' : 'Add Team Parties'}
        </h1>
      </div>

      <div className="bg-[#1C1C1C] rounded-xl border border-[#3A3530] w-full max-w-4xl overflow-hidden mx-auto">
        <TeamPartiesForm initialData={initialData} availableGames={availableGames} onClose={handleClose} onSave={handleSave} />
      </div>
    </div>
  );
};
