  ...
  const handleSave = (subItemData: Partial<HeaderSubItem>) => {
    ...
    categories = categories.map(category => {
      if (category.id === categoryId) {
        ...
        if (subItemId && subItemId !== 'new') {
          newSubItems = newSubItems.map(item => item.id === subItemId ? { ...item, ...subItemData, pageType: 'game' } as HeaderSubItem : item);
        } else {
          newSubItems.push({ ...subItemData, id: Date.now().toString(), isHidden: false, pageType: 'game' } as HeaderSubItem);
        }
  ...
  return (
    <div className="p-6 text-white min-h-screen">
      ...
      <div className="bg-[#1C1C1C] rounded-xl border border-[#3A3530] w-full max-w-4xl overflow-hidden mx-auto">
        <GameForm initialData={initialData} onClose={handleClose} onSave={handleSave} />
      </div>
    </div>
  );
};
